...
| 10 | **完整协议已实测跑通并拿到成绩**：11:42 的真实演练里 `--strategy matrix` **10/10 全清**（官方 `result.json` 的 `jammer_count=10`），wall 2.12 s、虚拟 4793.451 s（见 §3.4、§3.8）。 |
| 11 | **官方仿真参数已用请求流水反解并逐行验证**：移动 `直线距离/5.0 m/s` + 检测 5 s + 成功清除 5 s/失败清除 3 s + 切频道 1 s；182 行流水 181 行吻合（误差 <0.01 s），累计距离与虚拟时间与本地日志逐位一致（见 §3.9 与 `docs/data/online-p3-matrix-20260913-requestlog.tsv`）。 |
...
### 3.9 官方请求流水逐行复算：仿真参数反解与本案结构

原始流水（模拟器 GUI 日志页，182 行）存于
`docs/data/online-p3-matrix-20260913-requestlog.tsv`，含每步的时刻 / 指令 / 位置 / 频道 / 响应 / 虚拟时间。

**反解出的官方运动学（与附件2 §4.2–4.3 一致）**

| 规则 | 取值 | 复算依据 |
| --- | --- | --- |
| 移动耗时 | 直线距离 / **5.0 m/s** | 例：(1010,0) → (714.18,714.18) 距离 773.03 m，占用 154.61 s |
| `/measure` 动作 | **5 s** | 同点连续测量：`2307.712 → 2312.712` 恰好 +5.000 s |
| `/clear` 成功 | **5 s** | (223.20,1695.37) → (-460.69,1528.48) 703.96 m/5 = 140.79 s，加上 5 s 正好是 `4647.660 → 4793.451` |
| `/clear` 范围内无目标 | **3 s** | (-460.06,1519.16) → (-473.32,1544.81) 28.87 m/5 = 5.774 s + 3 s = `4074.738 → 4083.513` 的 8.775 s |
| 频道切换 | 仅当 `/measure` 的频道 ≠ 测向机当前频道时 **1 s**；`/clear` 不切频道 | 中心点首测 5 s、其后每步 6 s；同点同频道重测 +5.000 s |

**逐行复算结果（181/181 行吻合，误差 < 0.01 s）**

| 指标 | 流水复算 | 本地日志（`logs/runs/20260913T034232Z_s11_online-matrix_91a392`） |
| --- | --- | --- |
| 累计移动距离 | 18712.310 m | 18712.255 m（GUI 位置只显示 2 位小数，可解释） |
| 总虚拟时间 | 4793.451 s | 4793.451 s（逐位相同） |
| 测量 / 清除 / 成功 / 失败 / 切频道 | 167 / 12 / 10 / 2 / 160 | 167 / 12 / 10 / 2 / 160（完全一致） |

时间分解：**移动 3742.46 s（78.1%）** + 检测 835 s（17.4%）+ 切频道 160 s（3.3%）+ 清除 56 s（1.2%，10×5 + 2×3）= 4793.46 s。

> 这同时验证了三件事：官方 5 m/s 运动学、我们 mock 的常量（`mock/world.py` 的 `MOVE_SPEED_MPS` / `MEASURE_SECONDS`）、
> 以及客户端与统计口径（动作计数与官方流水逐项一致）。
> 唯一例外是流水里 (-714.18,-714.18) 的 ch17 显示 `1670.021 s`，按模型应为 `1664.021 s`（同一时刻两条）。
> 已在数据文件里按模型修正并注明。

复算脚本（可复用，正式测试每场都建议跑一遍）：

```bash
python3 tools/requestlog_kinematics.py docs/data/online-p3-matrix-20260913-requestlog.tsv \
  --run logs/runs/20260913T034232Z_s11_online-matrix_91a392
# 输出：逐行吻合率、动作计数、累计距离、时间分解，以及"本地日志 vs 官方流水"的逐项对照
```

它能在第一时间发现两类问题：**本地统计与官方流水不一致**（客户端/统计口径出 bug），
或**仿真参数被改动**（比如速度/动作耗时变了，用 `--speed` 等参数反推真实常量）。

**本案结构（可用于对照 mock 案例分布）**

- 案例 `ZD4M-9UQ8-AKZS-JTG6`：10 个源、**全为全向**、占用频道 {1,7,10,11,12,13,15,16,18,19}
- 中心 (0,0) 扫完 20 个频道（119 s）只探到 **2 个源**（ch7 222.71°、ch15 245.01°）→ 源普遍在有效半径之外
- 半径 1010 m 的 8 点环扫（polygon）从 119 s 跑到 2141.229 s = **2022 s，占全程 42%**，把 10 个源的方位都摸了一遍
- 逼近+清除段 2141 → 4793 s（2652 s）清掉全部 10 个源
- 单源代价差异极大：ch11/12/13/15 都是"一点测量 → 清除成功"；而 ch1 连测 5 点
  (`(-0.00,-1080)` → `(13.24,-1281.03)` → `(31.19,-2017.23)` → `(36.80,-1784.66)` → `(41.14,-1753.79)`) 才在 `(38.97,-1769.23)` 成功，
  其中 `(31.19,-2017.23)` 已是越过目标后的回折
- **ch10 的 2 次"范围内无目标"**（各 3 s）= 本地日志里的 `clear_failure_count=2`：
  在 `(-473.32,1544.81)` 两次都判 117.36°，说明**单点示向度反解出的位置不在清除范围内**；
  改到 `(-807.50,1398.63)`（20.20°）与 `(223.20,1695.37)`（194.21°）两点重新交会后，
  才在 `(-460.69,1528.48)` 清除成功 —— 这正是"两/三点交会后再清除"优于"单点沿向推进"的直接证据

**据此得到的两条调参启示**

1. **移动占 78% 虚拟时间，切频道只占 3.3%** → 优化重心在路线与逼近段，而不是少扫频道。
2. 环扫半径 1010 m 一次就吃掉 42% 预算，却只提供粗略方位；半径/点数的边际收益需要用同一套复算脚本在更多场次上评估。
