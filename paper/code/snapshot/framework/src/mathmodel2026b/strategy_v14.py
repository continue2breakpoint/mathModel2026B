...
v14 的结构修复：
1. **沿射线推进**（⚠️ **启发式，不是证明** —— 见文末"口径更正"）：设 A 为最近一次
   成功读数的顶点，u 为该读数的示向单位向量，源近似为 G = A + t_G·u。
   射线上的点 q(t) = A + t·u 在 t < t_G 时靠近源、t > t_G 时越过源，
   于是"步进 + no_signal 二分回退"通常能在 O(log) 次检测内把源距压到小区间。
   对全向源额外成立一条严格结论：无正面约束时 no_signal ⟺ 越过有效接收半径 R，
   因此它确实是上界。
...
⚠️ 口径更正（2026-09-13 独立复核 `review_20260913/审阅结论.md` §三）
--------------------------------------------------------------------
本模块此前把上述"沿射线推进"称作**正确性证明**。**该论证不成立。**

论证里用了"t < t_G 时 q 在源的正面、t > t_G 时在背面"，但它把**带 ±1° 误差的
测向射线**当成了穿过真源的直线。复核给出的显式反例：

    A = (0, 0)，G = (1000, 0)，接收半径 R = 1200m，源定向 φ = 90.1°。
    A 位于源正面；若读数偏差 −1°，沿该读数走 450m 得到 q ≈ (449.93, −7.85)。
    q 距真源约 550m，**却已经落在源的背面**（(q−G)·u(φ) < 0）。

也就是说，机器狗可能在**尚未接近源之前**就走到背面。此时 ``no_signal`` 并不意味着
越过真源，而 ``_walk_the_ray`` 里直接执行 ``hi = t``，会把真实距离上界从约 1000m
错误截断到 450m —— **把真值排除在保守可行域之外**。

实际后果（复核点名）：``all_directional 128 / ch11`` 的射线检测曾经走到距源 **15.9m**
处因背面返回 ``no_signal``，随后错误收缩上界，最终地毯清除点跑到 **240–258m** 外。

正确的读法：

* 射线推进**保留**（它在多数案例上确实快），但它的上下界**不是证明**，不得用作
  "必清"的依据，也不得污染保守二维可行域；
* ``no_signal`` 应按附件1 §2.2 的析取语义理解 —— "超出接收距离 **或** 落在定向
  覆盖角度范围外"；
* 真正的兜底交给与朝向无关的二维覆盖：见 :mod:`mathmodel2026b.knowledge_layer`
  与 :class:`~mathmodel2026b.strategy_v18.Q4V18Strategy`。

因此 ``q3-v18`` / ``q4-v18`` 在 v17 之上补了"失败 ``/clear`` 的 20m 排除圆 +
25m 格覆盖式清除 + 代价模型"这一层。完整记录见 ``docs/review-fixes-2026-09-13.md``。
...
class Q4V14Strategy(Q4V9Strategy):
    ...
    def _localize_and_clear(self, client: SimulatorClient, state: DogState, channel: int) -> None:
        ...
        # 2.5) 扩展点（默认空操作）：v18 在这里插入可证明的覆盖式清除。
        #      直接清失败后可行域通常已缩到"两三个 20m 圆就能盖住"，
        #      此时覆盖计划 (3k+2)s 严格便宜于"再走一趟补读数 + 再清一次"。
        if not ch_state.is_cleared and self._after_direct_clear_failed(
